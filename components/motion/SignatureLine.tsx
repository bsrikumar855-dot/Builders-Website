...
export type SignatureLineVariant = "circuit" | "pipe";
...
export default function SignatureLine({ variant = "circuit", morph = false, delay = 0, className }: SignatureLineProps) {
  ...
  const staticPath = variant === "circuit" ? CIRCUIT_PATH : PIPE_PATH;
  const staticStroke = variant === "circuit" ? VOLTAGE : COPPER;
  ...
  return (
    <div ref={containerRef} className={className}>
      ...
      >
        ...
        {/* Junction markers: squares (circuit nodes) <-> circles (pipe flanges) */}
        {JOINTS.map((p, i) => (
          <g key={i}>
            <motion.rect
              x={p.x - 4}
              ...
              height={8}
              fill={VOLTAGE}
              style={{ opacity: isMorphing ? jointOpacity : variant === "circuit" ? 1 : 0 }}
            />
            <motion.circle
              cx={p.x}
              cy={p.y}
              r={5}
              fill={COPPER}
              style={{ opacity: isMorphing ? flangeOpacity : variant === "pipe" ? 1 : 0 }}
            />
          </g>
        ))}
      </svg>
